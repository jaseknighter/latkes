...
Waveformer {
  ...
  var buffers; //one or more arrays of buffers
	...
	*new {
    arg buf_arrays,waveform_queue_sizes;
		^super.new.init(buf_arrays,waveform_queue_sizes);
    ...
    arg buf_arrays, waveform_queue_sizes;
    buffers=buf_arrays;
		(["init waveformer",waveform_queue_sizes,buffers]).postln;
    lua_sender = NetAddr.new("127.0.0.1",10111);   

    //create the slots for the waveformQueue
    waveformQueue = Array.new(waveform_queue_sizes.size);
    waveform_queue_sizes.do({ 
      arg array_size,i; 
      waveformQueue.insert(i, Array.new(array_size))
    });
	}
  ...
  queueWaveformGeneration {
    ...
    if(buffers[buf_array_ix][buf_ix].notNil, {
      item = (
        buf_array_ix: 0,
        buf_ix: 0,
        buf_array_ix: buf_array_ix,
        buf_ix: buf_ix,
        sample_start: sample_start,
        sample_length: sample_length
      );
      waveformQueue[buf_array_ix].addFirst(item);
	...
	stopWaveformGeneration {
		...
		firstId.for(lastId, {
			...
			removeQueueIndex = waveformQueue[buf_array_ix].detectIndex({
				arg item,i;
        item.buf_ix == i;
			});
      if((removeQueueIndex.notNil).and( waveformQueue[buf_array_ix].size > 1), {
        waveformQueue[buf_array_ix].removeAt(removeQueueIndex);
    ...
    arg buf_array_ix;
		...
		generatingWaveform = buf_array_ix.asString ++ "-" ++ waveformQueue[buf_array_ix].last.buf_ix.asString;

		waveformRoutine = Routine.new({

			while({ waveformQueue[buf_array_ix].notEmpty }, {
        ...
        var numFramesRemaining, numChannels, chunkSize, numSlices, sliceSize, stride, framesInSliceRemaining;
				...
				var item = waveformQueue[buf_array_ix].pop;
				var sampleId = buf_array_ix.asInteger.asString ++ "-" ++ item.buf_ix.asInteger.asString;
        ...
        if(buf.isNil, {
					...
					// Process in chunks
					while({
            (numFramesRemaining > 0).and(abandonCurrentWaveform == false)
  ...
  sendWaveform {
		...
		var padding = 0;
    var voice = sampleId.split($-)[0].asInteger;
    var mode = sampleId.split($-)[1].asInteger;
